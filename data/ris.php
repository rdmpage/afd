...
function process_ris_key($key, $value, &$obj)
{
	...
	switch ($key)
	{
		...
		case 'UR':
			...
			if (preg_match('/^http:\/\/dx.doi.org\/(?<doi>.*)$/', $value, $matches))
			{
				$obj->doi = $matches['doi'];
			}
			
			if (preg_match('/^http:\/\/doi.wiley.com\/(?<doi>.*)$/', $value, $matches))
			{
				$obj->doi = $matches['doi'];
			}

			if (preg_match('/^http:\/\/hdl.handle.net\/(?<hdl>.*)$/', $value, $matches))
			{
				$obj->hdl = $matches['hdl'];
			}

			if (preg_match('/^http:\/\/biostor.org\/reference\/(?<id>.*)$/', $value, $matches))
			{
				$obj->biostor = $matches['id'];
			}
			
			// PDFs
			if (preg_match('/^http:\/\/www.repository.naturalis.nl\/document\/(?<id>.*)$/', $value, $matches))
			{
				$obj->pdf = $value;
			}
			if (preg_match('/\.pdf$/', $value, $matches))
			{
				$obj->pdf = $value;
			}
