...
function output(doc, triples) {
  if (1) {
	  ...
	  for (var i in triples) {
	    ...
	    row.push(triples[i][s]); 
	    row.push(triples[i][p]); 
	    row.push(triples[i][o].replace(/"/g, '\"')); 
	  ...
	  html += '<pre>' + JSON.stringify(t, null, 2) + '</pre>';
...
//  "title" : "http://purl.org/dc/terms/title",
 ...
 "tname":"http://rs.tdwg.org/ontology/voc/TaxonName#",
 "tpub":"http://rs.tdwg.org/ontology/voc/PublicationCitation#",
 'tconcept' : 'http://rs.tdwg.org/ontology/voc/TaxonConcept#',
